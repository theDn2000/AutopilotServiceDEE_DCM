...
def change_altitude(self, altitude):
    ...
    print ("Dronelink function executed: change_altitude")
    self.reaching_waypoint = True
    
    self.vehicle.mav.send(
        mavutil.mavlink.MAVLink_set_position_target_global_int_message(1000, self.vehicle.target_system,
    ...
    # Wait for the vehicle to reach the desired altitude [absolute value of the difference between the current altitude and the desired altitude is less than 0.5 meters]
    while abs(int(self.alt) - int(altitude)) > 0.5:
        time.sleep(0.1)
...
def change_altitude_trigger(self, altitude, blocking=False):
    ...
    if blocking:
        ...
        ch = threading.Thread(target=change_altitude, args=(self, altitude))
